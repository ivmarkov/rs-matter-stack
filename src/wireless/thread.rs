...
use super::{Gatt, PreexistingWireless, WirelessBle};

/// A type alias for a Matter stack running over Thread (and BLE, during commissioning).
pub type ThreadMatterStack<'a, M, E = ()> = MatterStack<'a, WirelessBle<M, networks::Thread, E>>;

impl<M, E> MatterStack<'_, WirelessBle<M, networks::Thread, E>>
where
...
impl<M, E, H, X> GattTask for MatterStackWirelessTask<'static, M, networks::Thread, E, H, X>
where
    M: RawMutex + Send + Sync + 'static,
    E: Embedding + 'static,
    H: AsyncMetadata + AsyncHandler,
{
        ...
        P: GattPeripheral,
    {
        ...
        let net_ctl =
            NetCtlConnect::<NoopRawMutex, _>::new(NetCtlBtpOnly::new(NetworkType::Thread));
...
impl<M, E, H, X> ThreadTask for MatterStackWirelessTask<'static, M, networks::Thread, E, H, X>
where
    M: RawMutex + Send + Sync + 'static,
    E: Embedding + 'static,
    H: AsyncMetadata + AsyncHandler,
...
impl<M, E, H, X> ThreadCoexTask for MatterStackWirelessTask<'static, M, networks::Thread, E, H, X>
where
    M: RawMutex + Send + Sync + 'static,
    E: Embedding + 'static,
    H: AsyncMetadata + AsyncHandler,
