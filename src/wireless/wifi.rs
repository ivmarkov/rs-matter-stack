...
use rs_matter::data_model::networks::{
    self, NetChangeNotif, NetCtlBtpOnly, NetCtlConnect, WirelessMgr,
};
use rs_matter::data_model::objects::{AsyncMetadata, Endpoint};
use rs_matter::data_model::root_endpoint::{self, with_sys, with_wifi, SysHandler, WifiHandler};
...
use rs_matter::transport::network::btp::{Btp, GattPeripheral};
...
use super::{Gatt, GattTask, PreexistingWireless, WirelessBle};

/// A type alias for a Matter stack running over Wifi (and BLE, during commissioning).
pub type WifiMatterStack<'a, M, E = ()> = MatterStack<'a, WirelessBle<M, networks::Wifi, E>>;

impl<M, E> MatterStack<'_, WirelessBle<M, networks::Wifi, E>>
where
...
impl<U, N, C, P> Wifi for PreexistingWireless<U, N, C, P>
where
    U: UdpBind,
    N: NetifDiag + NetChangeNotif,
    C: NetCtl + WifiDiag + NetChangeNotif,
...
impl<U, N, C, P> WifiCoex for PreexistingWireless<U, N, C, P>
where
    U: UdpBind,
    N: NetifDiag + NetChangeNotif,
    C: NetCtl + WifiDiag + NetChangeNotif,
...
impl<M, E, H, U> GattTask for MatterStackWirelessTask<'static, M, networks::Wifi, E, H, U>
where
    M: RawMutex + Send + Sync + 'static,
    E: Embedding + 'static,
    H: AsyncMetadata + AsyncHandler,
{
        ...
        P: GattPeripheral,
    {
        ...
        let net_ctl = NetCtlConnect::<NoopRawMutex, _>::new(NetCtlBtpOnly::new(NetworkType::Wifi));

        let mut btp_task = pin!(self.0.run_btp(&btp, &net_ctl));
...
impl<M, E, H, X> WifiTask for MatterStackWirelessTask<'static, M, networks::Wifi, E, H, X>
where
    M: RawMutex + Send + Sync + 'static,
    E: Embedding + 'static,
    H: AsyncMetadata + AsyncHandler,
    X: UserTask,
{
        ...
        C: NetCtl + WifiDiag + NetChangeNotif,
    {
        ...
        let mut mgr = WirelessMgr::new(&self.0.network.networks, &net_ctl, &mut []); // TODO
        ...
        let handler = self.0.root_handler(&(), &netif, &net_ctl, &false, &self.1);
        let mut handler_task = pin!(self.0.run_handler((&self.1, handler)));
...
impl<M, E, H, X> WifiCoexTask for MatterStackWirelessTask<'static, M, networks::Wifi, E, H, X>
where
    M: RawMutex + Send + Sync + 'static,
    E: Embedding + 'static,
    H: AsyncMetadata + AsyncHandler,
    X: UserTask,
{
        ...
        G: GattPeripheral,
    {
        ...
        let handler = self.0.root_handler(&(), &netif, &net_ctl, &true, &self.1);
        let mut handler_task = pin!(self.0.run_handler((&self.1, handler)));
